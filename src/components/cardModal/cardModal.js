...
export default function CardModal({ workSpaceId, tags, card, setOpenCardModal, setCurrentCard, setDraggableBoard, boards }) {
    ...
    async function moveToBoard(boardId) {
        ...
        card.sortIndex = cardsInBoard.length > 0 ? cardsInBoard[cardsInBoard.length - 1].sortIndex + 1 : 1;
            ...
            <div className="card-modal" draggable={false}>
                <div className="card-modal__body">
                    <div className="card-modal__info">
                        ...
                        <div className="card-modal-caption" onClick={() => setEditCaption(true)} draggable={false}>
                            {!editCaption ? (
                            ...
                            <div className="card-modal-date">
                                ...
                                {!addDate ? (
                                    ...
                                    <div className="date__body">
                                        ...
                                        />
                                        <input value="OK" type="button" className="green" onClick={() => addCardDate()} />
                                        <input value="X" type="button" className="red" onClick={() => setAddDate(false)} />
                                    </div>
                                )}
                            </div>
                        ) : (
                        ...
                        {taskList.length || addTaskList > 0 ? <TaskList tasks={taskList} cardId={card.id} /> : ""}
                        <div className="card-modal-history">
                            ...
                            {history.length ? (
                                history.map((h) => {
                                    if (h.type === "move") {
                                        return (
                                            <div key={h.id} className="card-modal-history-move-container">
                                                <div className="card-modal-history-move-delay">{h.creatorAvatar !== null ? <img alt="avatar" className="card-modal-history-move-creator-avatar" src={h.creatorAvatar} /> : <div className="avatar-replacer">{AvatarUtils.getAvatarReplacerText(h.creatorName, h.creatorEmail)}</div>}</div>
                                        ...
                                        return (
                                            <div key={h.id} className="card-modal-history-comment">
                                                <div className="comment-text-container">
                                                    <div className="card-modal-history-comment-creator-avatar__container">{h.creatorAvatar !== null ? <img alt="avatar" className="card-modal-history-comment-creator-avatar" src={h.creatorAvatar} /> : <div className="avatar-replacer">{AvatarUtils.getAvatarReplacerText(h.creatorName, h.creatorEmail)}</div>}</div>
                                                    <div className="comment-creator">
                                                        <h2>{h.creatorName ? h.creatorName + `<${h.creatorEmail}>` : h.creatorEmail}</h2>
                                                                ...
                                                                if (file.type === "other file") {
                                                                    return (
                                                                        <div key={index} className="other-file">
                                                                            <img alt="file" className="other-file-image" src={fileImage} />
                                                        ...
                                                        <div className="comment-date">{formatter.format(h.date.toDate())}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        );
                    ...
                    <div className="card-modal__btn">
                        <div className="card-modal-users-tags-container">
                            <div className="card-modal-tags">
                                {tags.map((tag) => {
                                    return (
                                        <div key={tag.id} className="card-modal-tag" style={{ backgroundColor: tag.color }}>
                                            {tag.name}
                                        </div>
                                    );
                                })}
                        ...
                        <div className="card-modal-delay">
                            ...
                            />
                            {addUsers ? <AddUsers card={card} setCurrentCard={setCurrentCard} workspaceId={workSpaceId} setAddUser={setAddUsers} /> : ""}
                            <div className="btn-tags__container">
                                ...
                                />
                                {addTags ? <AddTags workSpaceId={workSpaceId} cardId={card.id} /> : ""}
                            </div>

                            <div className="move__container">
                                <input type="button" value="Переместить" onClick={() => setMoveInBoard(!moveInBoard)} />
                                {moveInBoard ? (
